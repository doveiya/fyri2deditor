...
{
    public class XnaDrawingBatch
    {
        private XnaDrawingRender render;
        public XnaDrawingRender Render
        {
            get
            {
                return this.render;
        ...
        #region Constructor
        public XnaDrawingBatch(GraphicsDevice graphicsDevice)
        {
            GraphicsDevice = graphicsDevice;
            this.render = new XnaDrawingRender(graphicsDevice);
        }

        public XnaDrawingBatch(RenderTarget2D renderTarget)
        {
            GraphicsDevice = renderTarget.GraphicsDevice;
            this.render = new XnaDrawingRender(renderTarget);
        }

        public XnaDrawingBatch(RenderTarget2D renderTarget, Texture2D background)
        {
            GraphicsDevice = renderTarget.GraphicsDevice;
            this.render = new XnaDrawingRender(renderTarget, background);
        }
        #endregion

        private const int MAX_ATOMIC_POLYGON_LENGTH = XnaDrawingRender.MAX_VERTEX_COUNT;
        ...
        private VertexPositionColor[] _lineVertexData = new VertexPositionColor[XnaDrawingRender.MAX_VERTEX_COUNT];
        private int _lineVertexCount;
        VertexBuffer _lineVertexDataBuffer;

        private short[] _lineIndexData = new short[XnaDrawingRender.MAX_INDEX_COUNT];
        private int _lineIndexCount;
        IndexBuffer _lineIndexBuffer;
        #endregion

        #region VertexBuffer for TriangleList
        private VertexPositionColor[] _triangleVertexData = new VertexPositionColor[XnaDrawingRender.MAX_VERTEX_COUNT];
        private VertexPositionNormalTexture[] _triangleVertexTextures = new VertexPositionNormalTexture[XnaDrawingRender.MAX_VERTEX_COUNT];
        private int _triangleVertexCount;
        VertexBuffer _triangleVertexDataBuffer;
        VertexBuffer _triangleVertexTextureBuffer;

        private short[] _triangleIndexData = new short[XnaDrawingRender.MAX_INDEX_COUNT];
