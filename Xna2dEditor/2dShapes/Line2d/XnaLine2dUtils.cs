...
namespace Fyri2dEditor
{
    public partial class XnaLine2d
    {
        ...
        {
            Vector2 v = EndPoint - StartPoint; // Vector from line's p0 to p1
            Vector2 w = p - StartPoint; // Vector from line's p0 to p
            ...
            {
                closestP = StartPoint;
                return Vector2.DistanceSquared(p, StartPoint);
            }
        ...
        public void FindFirstIntersection(Vector2 p0, Vector2 p1, float dist, out float tMin)
        {
            ...
            FindRadialT(p0, p1, this.StartPoint, dist, out t1, out t2);
            if (t1 < tMin && t1 >= 0.0f && t1 < 1.0f)
                tMin = t1;

            FindRadialT(p0, p1, this.EndPoint, dist, out t1, out t2);
            if (t1 < tMin && t1 >= 0.0f && t1 < 1.0f)
                tMin = t1;
